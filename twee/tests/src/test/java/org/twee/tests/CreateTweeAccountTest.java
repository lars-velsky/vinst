 ...
 * @since 24/12/14
 */
public class CreateTweeAccountTest {
    ...
    public void createAccountTest() throws ExecutionException, InterruptedException {
        CompletableFuture<CreateAccountResponse> createAccountFuture = core.process(new CreateAccountRequest());
        CreateAccountResponse createAccountResponse = createAccountFuture.get();
        ...
        CompletableFuture<GetAccountResponse> getAccountFuture = core.process(new GetAccountRequest(accountKey));
        GetAccountResponse getAccountResponse = getAccountFuture.get();

        Assert.assertNotNull("Null get account response", getAccountResponse);

        TweeAccount account = getAccountResponse.getAccount();

        Assert.assertNotNull("Null account in response", account);

        Assert.assertEquals("Created account's balance doesn't equal 0", account.getBalanceUSD(), 0, 0.001);
    }
